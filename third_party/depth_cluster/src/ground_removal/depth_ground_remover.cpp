...
void DepthGroundRemover::OnNewObjectReceived(const Cloud& cloud,
  ...
  const cv::Mat& depth_image_ =
      RepairDepthCol(cloud.projection_ptr()->depth_image(), 5, 1.0f);
  const cv::Mat& depth_image =
      RepairDepthRow(depth_image_, 5, 1.0f);
  Timer total_timer;
  auto angle_image = CreateAngleImage(depth_image);
  //auto smoothed_image = ApplySavitskyGolaySmoothing(angle_image, _window_size);
  no_ground_image = ZeroOutGroundBFS(depth_image, angle_image,
...
Mat DepthGroundRemover::ZeroOutGroundBFS(const cv::Mat& image,
  ...
  for (int r = 0; r < label_image_ptr->rows; ++r) {
    for (int c = 0; c < label_image_ptr->cols; ++c) {
      if (label_image_ptr->at<uint16_t>(r, c) == 0) {
...
Mat DepthGroundRemover::RepairDepthCol(const Mat& no_ground_image, int step,
...
Mat DepthGroundRemover::RepairDepthRow(const Mat& no_ground_image, int step,
                                    float depth_threshold) {
  Mat inpainted_depth = no_ground_image.clone();
  for (int c = 0; c < inpainted_depth.cols; ++c) {
    for (int r = 0; r < inpainted_depth.rows; ++r) {
