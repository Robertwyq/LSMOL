...
class LinearImageLabeler : public AbstractImageLabeler {
 public:
  ...
  ~LinearImageLabeler() override = default;
  ...
  void LabelOneComponent_MultiDiff(uint16_t label, const PixelCoord& start,
                         const std::vector<AbstractDiff*> diff_helpers, std::vector<float> thresholds) {
    // breadth first search
    std::queue<PixelCoord> labeling_queue;
    labeling_queue.push(start);
    // while the queue is not empty continue removing front point adding its
    ...
    while (!labeling_queue.empty()) {
      ...
      for (const auto& step : Neighborhood) {
        ...
        for (size_t i = 0; i < diff_helpers.size(); i++) {
          auto diff_helper = diff_helpers[i];
          float threshold = thresholds[i];

          auto diff = diff_helper->DiffAt(current, neighbor);
          if (diff_helper->SatisfiesThreshold(diff, threshold)) {
            labeling_queue.push(neighbor);
            break;
          }
        }
   ...
   * @brief      Calculates the labels running over the whole image.
   */
  void ComputeLabels_MultiDiff(std::vector<DiffFactory::DiffType> diff_types, std::vector<float> thresholds) override {
    _label_image =
        cv::Mat::zeros(_depth_image_ptr->size(), cv::DataType<uint16_t>::type);
    
    std::vector<std::unique_ptr<AbstractDiff>> diff_helper_ptrs;
    for (auto &diff_type : diff_types) {
      auto diff_helper_ptr =
        DiffFactory::Build(diff_type, _depth_image_ptr, &_params);
      diff_helper_ptrs.push_back(std::move(diff_helper_ptr));
    }
    ...
    for (int row = 0; row < _label_image.rows; ++row) {
      for (int col = 0; col < _label_image.cols; ++col) {
        ...
        std::vector<AbstractDiff*> diff_helper_ptrs_;
        for (auto &diff_helper_ptr : diff_helper_ptrs) {
          diff_helper_ptrs_.push_back(diff_helper_ptr.get());
        }

        LabelOneComponent_MultiDiff(label, PixelCoord(row, col), diff_helper_ptrs_, thresholds);
